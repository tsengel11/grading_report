...
$PAGE->set_url(new moodle_url('/blocks/grading_report/grade_details_dip_new.php'));
...
$cohorts=get_cohort('New Diploma');
// Creating the Cohort list on Grading report pages.

$unit_data = $DB->get_records('block_grading_report_units',['course'=>'Dip']);

foreach($cohorts as $cohort){
    $cohort->drop_downitem='<a class="dropdown-item" href="'.$CFG->wwwroot.'/blocks/grading_report/grade_details_dip_new.php?cohortid='.$cohort->id.'">'.$cohort->name.'</a>';
}

foreach($users as $user){
    $user->userlink=convert_userlink_without_td_general($user->id,$user->firstname,$user->lastname,$url,'grade_dip_new'); // The location of student dashboard
...
$templatecontext = (object)[
    'texttodisplay'=>'Diploma of Building and Construction (Building) 2021',
    'users'=>array_values($users),
    'cohorts'=>array_values($cohorts),
    'studentnumber'=>count($users),
    'groupname'=>$cohorts[$selected_groupid]->name
];
echo $OUTPUT->render_from_template('block_grading_report/report_dip_new',$templatecontext);
